...
class RiderTest {
    ...
    void setUp() {
        ...
        Mockito.when( order2.getId()).thenReturn((long) 3);
        Mockito.when( orderController.getStatus(order3.getId())).thenReturn(new Fulfilled());
    ...
    void setLocation() {
        ...
        assertThat(rider1.getLocationId(), is(1));
    }
    ...
    void checkStatusWhileOrderDelivered() {
        rider1.deliverOrder(order2.getId());
        assertTrue(rider1.getStatus()==RiderStatus.OCCUPIED);
        rider1.deliveryComplete(order2.getId());
        assertTrue(rider1.getStatus()==RiderStatus.AVAILABLE);
